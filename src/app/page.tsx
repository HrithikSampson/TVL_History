...
export default async function Home () {
  const data: ChainTVLData[] = await fetch(`${process.env.URL}/api/get_tvl_data`).then((res)=>res.json());
  const tvlHTML = data.map(async (obj)=>{
    try{
      const image = await fetch(`${process.env.URL}/get_image?text=${obj.name}:+${obj.amount}`);
